...
public class TicTacToe {
	...
	public void setFirst() {
		//Set who goes first;
		if(new Random().nextInt(2) == 0) curTurn = 'x';
		else curTurn = 'o';
	}
	...
	public boolean checkWinner() {
		if(s[0] == s[1] && s[0] == s[2] && s[0] != '-'){ //Top Left - Top Right 
			...
			setWinner(s[0]);
			return true; // Top Left - Top Right && Top Left - Bottom Left
		}
		...
		if(s[8] == s[5] && s[8] == s[2] && s[8] != '-') {// Top Right - Bottom Right
			s[8] = s[5] = s[2] = Character.toUpperCase(s[8]);
			setWinner(s[8]);
			return true; 
		}
		if((s[4] == s[1] && s[4] == s[7]) && s[4] != '-') {// Middle Vertical
			...
			setWinner(s[4]);
			return true; // Middle Vertical/Horizontal
		}
